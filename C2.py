...
# all valid tuples of a specific subgroup and were previously generated by C1.py.
...
def plot_convex_hull(points, color):
    ...
    if np.all(points[:, 0] == points[0, 0]):
        print("Convex hull skipped: all X-values are identical.")
        return

    # check for identical Y-values
    if np.all(points[:, 1] == points[0, 1]):
        print("Convex hull skipped: all Y-values are identical.")
        return
    ...
    try:
        ...
        print(f"Convex hull error skipped: {e}")



#  Prepare convex hull data storage 
convex_hull_data = []

# Compute and store the convex hull for each group
for (herkunft, ab_value), points in grouped_points.items():
    ...
    # skip collinear groups (all X identical)
    if np.all(points[:, 0] == points[0, 0]):
        print(f"Convex hull skipped (identical X) for Herkunft {herkunft}, AB {ab_value}")
        continue

    # skip collinear groups (all Y identical)
    if np.all(points[:, 1] == points[0, 1]):
        print(f"Convex hull skipped (identical Y) for Herkunft {herkunft}, AB {ab_value}")
        continue
